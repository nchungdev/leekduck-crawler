...
def should_run_scraper(name: str, settings: dict[str, Any]) -> bool:
    """Decide whether to run a scraper based on schedule field."""
    schedule = settings.get("schedule")

    # No schedule → run normally
    if not schedule:
        return True
    ...
    # ------------------------------------------------------
    # RaidNow schedule: only run during active hours
    # ------------------------------------------------------
    if schedule_type == "raid_now":
        active_hours = schedule.get("active_hours", [0, 23])
        start_h, end_h = active_hours
        local_h = get_local_hour_utc7()

        if start_h <= local_h <= end_h:
            print(f"[RUN] {name}: within active hours {start_h}-{end_h}", flush=True)
            return True
        else:
    ...
    # ------------------------------------------------------
    # default_12h: always run (GitHub Actions cron decides frequency)
    # ------------------------------------------------------
    if schedule_type == "default_12h":
        return True

    # Unknown schedule → allow by default
    return True
